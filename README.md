...
- **implement-task** — Implements one or more defined implementation tasks. Given a single ticket id, a comma-separated list of ticket ids, or task number(s) from an HTML implementation plan (path provided by you), it dispatches one agent per task in order — each building the task with test-driven development and KISS/YAGNI, then verifying it with an independent read-only reviewer that checks for unrequested or dangerous behavior and for side effects elsewhere in the product, before fixing any confirmed issues. When no requirement document or implementation plan exists, it works straight from the ticket or the description you provide, keeping the same standards.
- **create-html** — Creates HTML documents from a bundled compact template, so every HTML file has simple, consistent, readable styling. Fill in a title and content; the `<style>` block stays fixed and no external dependencies are added.
- **explain-in-html** — Explains a topic (typically work from the current session, like a requirement or the implementation just done) as a step-by-step HTML document built via `create-html`. Before delivery it runs four review passes: logical ordering start to finish, precision of terms (no vague words like "file" or "service"), removal of AI-slop wording, and Mermaid diagrams for any step-based or parallel process (rendered with a vendored local Mermaid library, no CDN).
