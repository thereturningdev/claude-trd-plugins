...
- **implement-task** — Implements one or more defined implementation tasks. Given a single ticket id, a comma-separated list of ticket ids, or task number(s) from an HTML implementation plan (path provided by you), it dispatches one agent per task in order — each building the task with test-driven development, KISS/YAGNI, an adversarial pass via `adversarial-tester`, and end-to-end functional testing where possible.
- **quick-implement-task** — One-shot variant of `implement-task` for small tasks and quick fixes that don't need a specification. Works straight from a rough idea (the prompt or a ticket) with no functional/non-functional requirement document, while keeping the same standards: test-driven development, KISS/YAGNI, an adversarial pass via `adversarial-tester`, and end-to-end functional testing where possible. For large or ambiguous work, define a requirement first and use `implement-task`.
- **verify-implementation** — Checks the work on the current branch against the requirement that drove it (the branch's linked ticket, or its `docs/<feature-slug>/requirement.html`). Launches a dynamic workflow that runs four analyses concurrently: whether the implementation meets the requirement, whether it slipped in unrequested or dangerous behavior, where the change could ripple into the rest of the product, and whether those ripples actually break or contradict anything. Produces a single SHIP / FIX-FIRST / REJECT verdict with `file:line` evidence. Read-only — it verifies and reports, it does not edit code.
